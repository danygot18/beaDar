...
const OverallSurvey = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await axios.get('/api/v1/allQuestions');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('/api/v1/allsubmit', formDataArray);
  ...
  return (
    <Fragment>
     <div className="survey-container" style={{ background: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '140vh', width: '90%', maxWidth: '800px', padding: '30px', border: '1px solid #ccc', borderRadius: '6px', margin: '20px auto 0' }}>
